		...
		if(mc.renderViewEntity != null){
			...
			if(ray != null){
				...
				if(block != null && TCEApi.exchange.containsKey(block)){
					...
					int x = event.resolution.getScaledWidth()/2 + event.resolution.getScaledWidth()/3 + 50;
					int y = event.resolution.getScaledHeight()/2 - event.resolution.getScaledHeight()/3 - 35;
